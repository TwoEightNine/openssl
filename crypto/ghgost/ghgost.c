...
static const unsigned char C[32][GHGOST_BLOCK_SIZE] = {
...
void print_ghgost_key(GHGOST_KEY key) {
    printf("\nGHGOST key:");
...
void ghgost_expand_keys(
    ...
    memcpy(key[0], key_1, GHGOST_BLOCK_SIZE);
    memcpy(key[1], key_2, GHGOST_BLOCK_SIZE);
...
void ghgost_encrypt(
    ...
    memcpy(out, in, GHGOST_BLOCK_SIZE);
    invert_block(out);
...
void GHGOST_set_key(const unsigned char *userKey, const int bits,
    ...
    char key2[GHGOST_BLOCK_SIZE];
    memcpy(key1, &userKey[0], GHGOST_BLOCK_SIZE * sizeof(*userKey));
    memcpy(key2, &userKey[GHGOST_BLOCK_SIZE], GHGOST_BLOCK_SIZE * sizeof(*userKey));
    invert_block(key1);
    invert_block(key2);
